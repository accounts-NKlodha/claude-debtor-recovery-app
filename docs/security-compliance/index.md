...
- Authentication (V1 decision, supersedes this checklist's original Google-sign-in-allow-list-for-staff / client-OTP proposal — see `docs/DEPLOYMENT.md` §"Authentication" and `docs/authorization-hardening/index.md`): Supabase email + password for every role (staff, admin, client). Google OAuth is deferred, not part of V1, and not configured in any environment.
