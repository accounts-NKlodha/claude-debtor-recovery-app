/**
 * TanStack Start adapter for src/components/screens/msme-wizard.tsx --
 * identical UX/copy/stage logic/evidence-gate. useActionState/<form
 * action> -> local state calling msme.functions.ts directly;
 * next/navigation's router.refresh() -> TanStack Router's
 * router.invalidate({ sync: true }). The original's "auto-build preview
 * when advancing into the Preview stage via a hidden form" is preserved
 * as a direct async call inside `next()` once the step actually becomes
 * "preview" -- same one-call-per-advance behavior without the hidden-
 * form/requestSubmit workaround that pattern needed under useActionState.
 */
"use client";
...
export function MsmeWizard({ seed }: { seed: MsmeSeed }) {
  ...
  const [previewState, setPreviewState] = React.useState<BuildMsmePreviewState>(BUILD_MSME_PREVIEW_IDLE);
  const [ackState, setAckState] = React.useState<CaptureMsmeAcknowledgementState>(CAPTURE_MSME_ACK_IDLE);
  ...
  const next = async () => {
    ...
    if (STAGE_KEYS[nextStep] === "preview") {
      try {
        const result = await buildMsmePreviewFn({ data: { caseId: seed.caseId } });
        setPreviewState(result);
      } catch {
        setPreviewState({ result: null, error: "Failed to build the preview snapshot" });
      }
    }
  };
  const prev = () => {
  ...
  async function handleSubmitFiling(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setAckPending(true);
    try {
      const result = await captureMsmeAcknowledgementFn({ data: { caseId: seed.caseId } });
      setAckState(result);
      if (result.result?.diaryNumber) {
        await router.invalidate({ sync: true });
      }
    } catch {
      setAckState({ result: null, error: "Failed to submit the filing" });
    } finally {
      setAckPending(false);
    }
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {step < STAGES.length - 1 ? (
          <Button onClick={() => void next()} disabled={locked}>
            Next <ChevronRight className="h-3.5 w-3.5" />
          </Button>
        ) : (
          <form onSubmit={handleSubmitFiling}>
            <Button type="submit" disabled={locked || ackPending}>
              {locked ? "Submitted" : ackPending ? "Submitting…" : "Submit filing"}
            </Button>
          </form>
        )}
        <form onSubmit={handleSave}>
          <Button variant="ghost" type="submit" disabled={locked || savePending}>
            <Save className="h-3.5 w-3.5" /> {savePending ? "Saving…" : "Save & resume later"}
          </Button>
        </form>
