/**
 * TanStack Start adapter for src/components/app-shell.tsx -- identical
 * markup/behavior/Tailwind classes, only three things swapped:
 *   - `next/link`'s <Link href> -> TanStack Router's <Link to>
 *   - `usePathname` (next/navigation) -> useLocation (@tanstack/react-router)
 *   - signOutAction (Next Server Action) -> signOutFn (TanStack server fn),
 *     followed by a client-side navigate to /sign-in (the Next version's
 *     redirect() throws a special Next redirect signal server-side; a
 *     TanStack server function can't do that across the RPC boundary the
 *     same way, so the navigate happens after the call resolves instead).
 * Only exists on the TanStack port branch; the Next.js original is
 * untouched and still used by every existing Next.js page.
 */
  ...
  return (
    <div className="flex min-h-screen w-full flex-col">
      ...
      <header className="sticky top-0 z-40 flex h-14 items-center gap-2 border-b border-border bg-card px-3 sm:px-4">
        ...
        <Link to={surface === "internal" ? "/dashboard" : "/client"} className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-1 sm:ml-2">
          <ThemeToggle />
          <DropdownMenu trigger={<Avatar name={user.displayName} />}>
            <DropdownMenuLabel>
              {user.displayName}
              {user.email ? (
      ...
      <div className="flex flex-1">
        {/* Desktop sidebar */}
        <aside className={cn("hidden shrink-0 border-r border-border bg-card lg:block", collapsed ? "w-14" : "w-60")}>
