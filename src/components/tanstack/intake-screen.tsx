 ...
 * FormData is read from the form element itself on submit and converted
 * to a plain object, then reset via formRef.current?.reset() on success,
 * same mechanic as the original's action-dispatch + reset.
 */
"use client";
...
function ManualInvoiceForm({ organisationId }: { organisationId: string }) {
  const [state, setState] = React.useState<CreateManualInvoiceState>(CREATE_MANUAL_INVOICE_IDLE);
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const fields = Object.fromEntries(formData) as Record<string, unknown>;
      const result = await createCaseFromManualInvoiceFn({ data: { organisationId, ...fields } });
      setState(result);
      if (result.result) formRef.current?.reset();
    } catch {
      setState({ result: null, error: "Failed to create the draft case" });
    } finally {
      setPending(false);
    }
  }
...
/** Remounted (via the parent's `key`) on every new file load -- resets the
 * commit result/pending state cleanly, so committing file A doesn't leave
 * a stale "Committed" message showing once file B is loaded. */
function CommitPanel({
  ...
  const [commitState, setCommitState] = React.useState<CommitBulkImportState>(COMMIT_BULK_IMPORT_IDLE);
  const [pending, setPending] = React.useState(false);
  const committedCount = commitState.result?.casesCreated ?? null;

  async function handleCommit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setPending(true);
    try {
      const commitResult = await commitBulkImportFn({ data: { organisationId, csvText } });
      setCommitState(commitResult);
    } catch {
      setCommitState({ result: null, error: "Failed to commit the import" });
    } finally {
      setPending(false);
    }
  }
  ...
  return (
    <form onSubmit={handleCommit} className="mt-3 flex items-center gap-3">
      <Button type="submit" disabled={result.validRows === 0 || pending || committedCount !== null}>
        {pending ? "Committing…" : committedCount !== null ? "Committed" : `Commit ${result.validRows} valid rows`}
      </Button>
      {committedCount !== null ? (
        <span className="inline-flex items-center gap-1 text-xs text-success">
...
function BulkImport({ organisationId }: { organisationId: string }) {
  ...
  const [validateState, setValidateState] = React.useState<ValidateBulkImportState>(VALIDATE_BULK_IMPORT_IDLE);
  const [validatePending, setValidatePending] = React.useState(false);

  async function validate(text: string) {
    setValidatePending(true);
    try {
      const validated = await validateBulkImportFn({ data: { csvText: text } });
      setValidateState(validated);
    } catch {
      setValidateState({ result: null, error: "Failed to validate the file" });
    } finally {
