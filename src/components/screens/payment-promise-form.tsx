...
import { recordPaymentPromiseAction, type RecordPaymentPromiseState } from "@/app/actions/whatsapp";
...
export function PaymentPromiseForm({
  ...
  const [state, formAction] = useActionState<RecordPaymentPromiseState, FormData>(recordPaymentPromiseAction, IDLE);
  const [promisedOn, setPromisedOn] = React.useState("");
  const [amount, setAmount] = React.useState("");
  const [invoiceId, setInvoiceId] = React.useState("");

  // Reset the fields once when a NEW "saved" result arrives. Done during
  // render (React's supported "adjust state on change" pattern), not in an
  // effect, so an error result keeps what the operator typed.
  const [handled, setHandled] = React.useState<RecordPaymentPromiseState>(IDLE);
  if (state.kind === "saved" && state !== handled) {
    setHandled(state);
    setPromisedOn("");
    setAmount("");
  }
  ...
  React.useEffect(() => {
    if (state.kind === "saved") router.refresh();
  }, [state, router]);

  return (
    <form action={formAction} className="flex flex-col gap-2 rounded-md border border-border px-3 py-2">
      <p className="text-sm font-medium">Record a promise to pay</p>
      <input type="hidden" name="caseId" value={caseId} />
      {sourceReplyId ? <input type="hidden" name="sourceReplyId" value={sourceReplyId} /> : null}
