-- 0019_email_delivery_fix_nested_call.sql
-- Live-only finding (email-delivery task, verification pass): 0018's
-- complete_delivery_attempt() called `select apply_case_mutation(...) into
-- v_case` (a nested RPC call capturing a composite return) -- live-
-- confirmed this fails with "invalid input syntax for type uuid" against
-- the row's own text representation, while calling apply_case_mutation
-- directly (not nested) works correctly. Root cause not pursued further
-- (not worth the risk of a second live-only PL/pgSQL edge case on a
-- production database); fixed instead by matching the pattern every other
-- RPC in this codebase already uses -- inline the same recovery_cases
-- UPDATE directly, rather than calling apply_case_mutation from inside
-- another SECURITY DEFINER function. No behavior change versus the
-- intended design in docs/email-delivery/index.md: the case-state update
-- and audit event this produces are identical in effect, only the SQL
-- mechanism changed.
